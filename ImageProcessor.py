"""
	...
	it process the images by turning them into a numpy array which can be used for analysis
...
class ImageProcessor(object):
	"""
		Args:
			folderPath(str): A string to the folder path that images will be pulled from
	"""	
	...
	def ImageToArray(self,imagePath):
		"""
			Args:
				imagePath(str): The path to the image that is to be converted to a numpy array
			This function takes in a path to an image and returns the a numpy array for that image
		"""
	...
	def processFolderImages(self):
		"""
			This function selects the folderPath memeber variable and turns all images in that
			file into a numpy array which is storred in the member variable processedImages
		""" 
